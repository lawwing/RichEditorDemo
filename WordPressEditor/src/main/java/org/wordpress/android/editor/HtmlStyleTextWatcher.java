...
public class HtmlStyleTextWatcher implements TextWatcher
{
        ...
        if (s.length() > lastCharacterLocation)
        {
            if (count > 0)
            {
                ...
                else
                {
                    ...
                    mOffset = start;
                    mModifiedText = s.subSequence(start + before,
                            start + count);
                }
            }
        }
    ...
    public void afterTextChanged(Editable s)
    {
        ...
        // If the modified text included a tag or entity symbol ("<", ">", "&"
        // or ";"), find its match and restyle
        ...
        {
            // If the modified text didn't include any tag or entity symbols,
            // restyle if the modified text is inside
            // a tag or entity
     ...
     * '&'), whether added or deleted, returns the range of text which should
     * have its style reapplied.
     * 
     ...
     */
    protected SpanRange getRespanRangeForChangedOpeningSymbol(Editable content,
            String openingSymbol)
    {
        ...
        int firstOpeningTagLoc = mOffset
                + mModifiedText.toString().indexOf(openingSymbol);
        int closingTagLoc;
        if (mLastOperation == Operation.INSERT)
        {
            // Apply span from the first added opening symbol until the closing
            // symbol in the content matching the
            // last added opening symbol
            // e.g. pasting "<b><" before "/b>" - we want the span to be applied
            // to all of "<b></b>"
            int lastOpeningTagLoc = mOffset
                    + mModifiedText.toString().lastIndexOf(openingSymbol);
            closingTagLoc = content.toString().indexOf(closingSymbol,
                    lastOpeningTagLoc);
        }
        else
        {
            // Apply span until the first closing tag that appears after the
            // deleted text
     ...
     * ';') and no opening symbols, whether added or deleted, returns the range
     * of text which should have its style reapplied.
     * 
     * @param content the content after modification
     * @param closingSymbol the closing symbol recognized (e.g. '>' or ';')
     * @return the range of characters to re-apply spans to
     */
    protected SpanRange getRespanRangeForChangedClosingSymbol(Editable content,
            String closingSymbol)
    {
        ...
        int firstClosingTagInModLoc = mOffset
                + mModifiedText.toString().indexOf(closingSymbol);
        int firstClosingTagAfterModLoc = content.toString()
                .indexOf(closingSymbol, mOffset + mModifiedText.length());
        
        int openingTagLoc = content.toString().lastIndexOf(openingSymbol,
                firstClosingTagInModLoc - 1);
        if (openingTagLoc >= 0)
        {
            if (firstClosingTagAfterModLoc >= 0)
            {
                return new SpanRange(openingTagLoc,
                        firstClosingTagAfterModLoc + 1);
            }
            else
            {
            ...
            String openingSymbol)
    {
        ...
        if (openingTagLoc >= 0)
        {
            int closingTagLoc = content.toString().indexOf(closingSymbol,
                    openingTagLoc);
    ...
    /**
     * Clears and re-applies spans to {@code content} within range
     * {@code spanRange} according to rules in {@link HtmlStyleUtils}.
     * 
